...
class _BloodBanksState extends State<BloodBanks> {
      ...
      theme: ut.maintheme(),
        ...
        appBar: AppBar(
          ...
          leading: IconButton(
              ...
              onPressed: () {if(index == 2 || index == 3){setState(() {
                index = 1;
              });} else {Navigator.pop(context);}}),
        actions: <Widget>[
        g.g_l.isEmpty ? SizedBox(height:1) :  index == 3 ?
          IconButton(icon: Icon(Icons.delete,color: Colors.white,),color: Colors.white, onPressed: (){
          deleteimage();
          })
            : IconButton(icon: Icon(Icons.add,color: Colors.white,),color: Colors.white, onPressed: (){
            setState(() {
              index=2;
            });
          })
  ...
  void _update() async {
   if(nameedit.text == "" || locationedit.text == "" || contactedit.text == "" ){
   ...
   var gallery = db.collection("bloodbanks");
    await gallery.findAndModify(
     query:{"_id":edit['_id']},update:{ "name": nameedit.text,"location":locationedit.text,"contact":contactedit.text}).then((
          onValue) {
  ...
  void _upload() async {
   ...
   var gallery = db.collection("bloodbanks");
    await gallery.insert({ "name": name.text,"location":location.text,"contact":contact.text}).then((
          onValue) {
      ...
      child: Container(
            ...
            child: Card(
              ...
              margin: EdgeInsets.all(10),
               ...
               children:[ TextField(autofocus:false,controller: nameedit, decoration: InputDecoration(labelText: "Name"),),
               TextField(autofocus:false,controller: locationedit, decoration: InputDecoration(labelText: "Location"),),
               TextField(autofocus:false,controller: contactedit, decoration: InputDecoration(labelText: "Contact"),),
               SizedBox(height:10),
               RaisedButton(
              ...
              margin: EdgeInsets.all(10),
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            mainAxisSize: MainAxisSize.min,
               children:[ TextField(autofocus:false,controller: name, decoration: InputDecoration(labelText: "Name"),),
               TextField(autofocus:false,controller: location, decoration: InputDecoration(labelText: "Location"),),
               TextField(autofocus:false ,controller: contact, decoration: InputDecoration(labelText: "Contact"),),
               SizedBox(height:10),
               RaisedButton(
                 padding: EdgeInsets.symmetric(vertical:10,horizontal: 45),
                 color:Colors.red,
